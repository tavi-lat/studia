...
const KEY="planner-v2";
const uid=()=>Date.now().toString(36)+Math.random().toString(36).slice(2);
const iso=d=>`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
const today=()=>iso(new Date());
const esc=s=>String(s??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#39;"}[c]));
let db=JSON.parse(localStorage.getItem(KEY)||"null")||{tasks:[],subjects:[],habits:[],reminders:[],schedule:[]};
...
function renderTasks(){
 ...
 list.sort((a,b)=>a.date.localeCompare(b.date)||(a.time||"99").localeCompare(b.time||"99"));
 $("#taskList").innerHTML=list.length?list.map(t=>`<article class="task">
 <button class="check ${t.done?"done":""}" data-toggle-task="${t.id}"></button>
 <div class="task-main" data-edit-task="${t.id}"><strong>${esc(t.title)}</strong><div class="meta">${formatDate(t.date)}${t.time?" · "+t.time:""}${t.subject?" · "+esc(subjectName(t.subject)):""}</div></div>
 <span class="badge">${esc(t.priority)}</span></article>`).join(""):`<div class="empty">No hi ha tasques en aquesta vista.</div>`;
}
...
function renderCalendar(){
 ...
 if(calendarView==="month"){
  ...
  const ts=db.tasks.filter(t=>t.date===id).sort((a,b)=>(a.time||"99").localeCompare(b.time||"99"));
  c.innerHTML=ts.length?ts.map(t=>`<article class="task" data-edit-task="${t.id}"><div class="task-main"><strong>${esc(t.title)}</strong><div class="meta">${esc(t.time||"Tot el dia")} · ${esc(subjectName(t.subject)||"General")}</div></div></article>`).join(""):`<div class="empty">No hi ha res programat.</div>`;
 }
}
...
function renderSubjects(){
 $("#subjectsList").innerHTML=db.subjects.length?db.subjects.map(s=>{
  ...
  ${tasks.length?tasks.map(t=>`<div class="subject-task" data-edit-task="${t.id}"><strong>${esc(t.title)}</strong><div class="meta">${formatDate(t.date)} · ${t.done?"Completada":"Pendent"}</div></div>`).join(""):'<div class="empty">Sense tasques.</div>'}</article>`;
 }).join(""):'<div class="empty">Crea assignatures per ordenar les tasques.</div>';
}

function renderHabits(){
 const now=new Date(),y=now.getFullYear(),m=now.getMonth(),daysIn=new Date(y,m+1,0).getDate(),firstDay=(new Date(y,m,1).getDay()+6)%7;
 $("#habitsList").innerHTML=db.habits.length?db.habits.map(h=>{
  ...
  return `<article class="habit-card"><div class="habit-head"><div><h3>${esc(h.name)}</h3><div class="meta">${new Intl.DateTimeFormat("ca-ES",{month:"long",year:"numeric"}).format(now)}</div></div><button class="text-btn" data-delete-habit="${h.id}">Eliminar</button></div>
...
function renderReminders(){
 ...
 const items=db.reminders.filter(r=>r.date>=m&&r.date<=e).sort((a,b)=>a.date.localeCompare(b.date)||(a.time||"99").localeCompare(b.time||"99"));
 $("#remindersList").innerHTML=items.length?items.map(r=>`<article class="reminder-card"><div class="reminder-day">${formatDate(r.date,{weekday:"long",day:"numeric",month:"long"})}</div><strong>${esc(r.title)}</strong><div class="meta">${esc(r.time||"Sense hora")}</div><button class="text-btn" data-delete-reminder="${r.id}">Eliminar</button></article>`).join(""):'<div class="empty">No hi ha recordatoris aquesta setmana.</div>';
}
...
function renderSchedule(){
 ...
 $("#scheduleGrid").innerHTML=days.map((day,i)=>{
  ...
  return `<div class="schedule-col"><h3>${day}</h3>${items.length?items.map(x=>`<article class="schedule-item"><span class="schedule-time">${esc(x.start)}–${esc(x.end)}</span><strong>${esc(x.name)}</strong><div class="meta">${esc(x.place||"")}</div><button class="text-btn" data-delete-schedule="${x.id}">×</button></article>`).join(""):'<div class="meta">Sense classes</div>'}</div>`;
 }).join("");
}
...
function openTask(t=null,date=selectedDate){
 $("#taskDialogTitle").textContent=t?"Editar tasca":"Nova tasca";$("#taskId").value=t?.id||"";$("#taskTitle").value=t?.title||"";$("#taskDate").value=t?.date||date;$("#taskTime").value=t?.time||"";$("#taskSubject").value=t?.subject||"";$("#taskPriority").value=t?.priority||"Normal";$("#taskComments").value=t?.comments||"";$("#deleteTaskBtn").classList.toggle("hidden",!t);$("#taskDialog").showModal()
}
...
$("#quickAdd").onclick=$("#addTaskBtn").onclick=()=>openTask();
...
$("#taskForm").onsubmit=e=>{e.preventDefault();const id=$("#taskId").value,obj={id:id||uid(),title:$("#taskTitle").value.trim(),date:$("#taskDate").value,time:$("#taskTime").value,subject:$("#taskSubject").value,priority:$("#taskPriority").value,comments:$("#taskComments").value,done:false};if(id){const old=db.tasks.find(x=>x.id===id);obj.done=old.done;db.tasks=db.tasks.map(x=>x.id===id?obj:x)}else db.tasks.push(obj);save();$("#taskDialog").close()};
...
$("#addReminderBtn").onclick=()=>openSimple("Nou recordatori",[{name:"title",label:"Títol",required:true},{name:"date",label:"Data",type:"date",value:today(),required:true},{name:"time",label:"Hora",type:"time"}],v=>{db.reminders.push({id:uid(),...v});save()});
...
$("#fullscreenBtn").onclick=()=>{$(".clock-panel").classList.toggle("fullscreen");$("#fullscreenBtn").textContent=$(".clock-panel").classList.contains("fullscreen")?"Sortir":"Pantalla completa"};
